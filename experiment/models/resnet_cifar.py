...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            # Option A: Zero-pad channels for dimension matching
            if identity.size(1) != out.size(1):
                pad = torch.zeros(identity.size(0), out.size(1) - identity.size(1),
                                identity.size(2), identity.size(3), 
                                device=identity.device, dtype=identity.dtype)
                identity = torch.cat((identity, pad), 1)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        self.bn1 = nn.BatchNorm2d(16)  # Changed from GroupNorm
        self.relu = nn.ReLU(inplace=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1:
            # Option A: Just downsample spatially, zero-pad channels in forward
